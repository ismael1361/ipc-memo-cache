...
export class WebSocketStructure extends ConnectionStructure {
	...
	private eventSendProcess: (message: PipeDataInfo) => void;

	constructor({ host = "localhost", port = 8080, isMaster = false, maxReconnectAttempts = 10 }: { host?: string; port?: number; isMaster?: boolean; maxReconnectAttempts?: number }) {
		super(true, isMaster);
	...
	mountMaster() {
		...
		wss.on("connection", (ws: WebSocket) => {
			...
			ws.on("message", (data: string) => {
				const message = JSON.parse(data) as PipeDataInfo;
