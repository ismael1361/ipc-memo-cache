    ...
    receive(data, receive) {
        ...
        return new Promise((resolve, reject) => {
            const processData = (message) => {
                if ([utils_1.list_event_type.PIPE_IN_PROCESS].includes(message.type)) {
                    resolve(undefined);
                }
                else {
                    resolve(message);
    ...
    send(message, send, attempts = 0) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                return this.send({ ...message, process_id: (0, utils_1.uuidv4)() }, send, attempts + 1)
                    .then(resolve)
                    .catch(reject);
            }
            return resolve();
